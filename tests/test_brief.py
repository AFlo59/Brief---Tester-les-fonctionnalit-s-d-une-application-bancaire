from database.Brief.bank import Transaction


def test_deposit_normal(account_brief_factory, my_brief_session):
    """Simple test with positive amount"""
    with my_brief_session:
        account = account_brief_factory(
            account_id = 1,
            balance = 100
        )
        account.deposit(50)
        ...
        assert my_brief_session.query(Transaction).count() == 1
        assert (my_brief_session
                .query(Transaction)
...
def test_deposit_negative_amount(account_brief_factory, my_brief_session):
    with my_brief_session:
        ...
        assert my_brief_session.query(Transaction).count() == 0
        # 3. Verify that session.commit wasn't called
        assert my_brief_session.commit.call_count == 1

def test_deposit_zero_amount(account_brief_factory, my_brief_session):
    with my_brief_session:
        account = account_brief_factory(
            account_id = 1,
            balance = 100
...
def test_withdraw_normal(account_brief_factory, my_brief_session):
    with my_brief_session:
        account = account_brief_factory(
            account_id = 1,
            balance = 100
...
def test_withdraw_insufficient_funds(account_brief_factory, my_brief_session):
    with my_brief_session:
        account = account_brief_factory(
            account_id = 1,
            balance = 100
...
def test_withdraw_negative_amount(account_brief_factory, my_brief_session):
    with my_brief_session:
        account = account_brief_factory(
            account_id = 1,
            balance = 100
...
def test_withdraw_zero_amount(account_brief_factory, my_brief_session):
    with my_brief_session:
        account = account_brief_factory(
            account_id = 1,
            balance = 100
...
def test_transfer_normal(account_brief_factory, my_brief_session):
    with my_brief_session:
        account1 = account_brief_factory(
            account_id = 1,
            balance = 100
        )
        account2 = account_brief_factory(
            account_id = 1,
            balance = 50
        )
        ...
        assert my_brief_session.query(Transaction).count() == 2
        results = (my_brief_session
                   .query(Transaction)
                   .all())
        ...
        # 4. Verify session.commit has been called
        assert my_brief_session.commit.call_count == 3  # 2 for accounts, 1 for both transactions

def test_transfer_insufficient_funds(account_brief_factory, my_brief_session):
    with my_brief_session:
        account1 = account_brief_factory(
            account_id = 1,
            balance = 100
        ...
        assert my_brief_session.query(Transaction).count() == 0
        # 3. Verify that session.commit wasn't called
        assert my_brief_session.commit.call_count == 2  # One for each account        

def test_transfer_negative_amount(account_brief_factory, my_brief_session):
    with my_brief_session:
        account1 = account_brief_factory(
            account_id = 1,
            balance = 100
...
def test_transfer_zero_amount(account_brief_factory, my_brief_session):
    with my_brief_session:
        account1 = account_brief_factory(
            account_id = 1,
            balance = 100
...
def test_get_balance_initial(account_brief_factory, my_brief_session):
    with my_brief_session:
        account1 = account_brief_factory(
            account_id = 1,
        )
...
def test_get_balance_after_deposit(account_brief_factory, my_brief_session):
    with my_brief_session:
        account = account_brief_factory(
            account_id = 1,
            balance = 200
...
def test_get_balance_after_withdrawal(account_brief_factory, my_brief_session):
    with my_brief_session:
        account = account_brief_factory(
            account_id = 1,
            balance = 400
...
def test_get_balance_after_failed_withdrawal(account_brief_factory, my_brief_session):
    with my_brief_session:
        account = account_brief_factory(
            account_id = 1,
            balance = 400
...
def test_get_balance_after_transfer(account_brief_factory, my_brief_session):
    with my_brief_session:
        account1 = account_brief_factory(
            account_id = 1,
            balance = 400
