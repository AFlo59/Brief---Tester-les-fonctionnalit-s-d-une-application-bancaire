...
# Test de transaction
def test_transaction(session, account_factory):
    # Créer deux comptes pour la transaction
    account_source = account_factory(balance=100.0)
    account_target = account_factory(balance=50.0)

    # Effectuer une transaction
    transaction_amount = 20.0
    transaction = Transaction()
    withdrawal, deposit = transaction.transfer(session, transaction_amount, account_source, account_target)

    # Vérifier que la transaction s'est déroulée correctement
    assert withdrawal.amount == transaction_amount
    assert deposit.amount == transaction_amount
    assert withdrawal.type == "Withdraw"
    assert deposit.type == "Deposit"
    assert account_source.balance == 100.0 - transaction_amount
    assert account_target.balance == 50.0 + transaction_amount
